package de.mmm.survival.dynmap;
...
public class DynmapWorldGuardPlugin_1_13 {
   ...
   * @param plugin Plugin
   */
  public DynmapWorldGuardPlugin_1_13(final JavaPlugin plugin) {
    this.plugin = plugin;
  ...
  private String formatInfoWindow(final ProtectedRegion region, final AreaMarker m) {
    ...
    if (m.getLabel().startsWith("P-")) {
      ...
      for (final UUID uuid : region.getOwners().getUniqueIds()) {
        if (owner == null) {
          owner = new StringBuilder(Survival.getInstance().async.getMySQL().getName(uuid));
        } else {
          owner.append(", ").append(Survival.getInstance().async.getMySQL().getName(uuid));
        }
      }
  ...
  /**
   * Fuegt einen Style hinzu
   *
   * @param resid   res Identifier
   * @param worldid Welt Identifier
   * @param m       Marker
   * @param region  Region
   */
  ...
  private void handleRegion(final World world, final ProtectedRegion region, final Map<String, AreaMarker> newmap) {
    ...
    if (isVisible(region.getId(), world.getName())) {
      ...
      if (tn == RegionType.CUBOID) { /* Cubiod region? */
        ...
        IntStream.range(0, points.size()).forEachOrdered(i -> {
