...
public class Hotbar {
   ...
   * @param p Spieler
   */
  public static void send(final Player p) throws ClassNotFoundException, NoSuchMethodException {
    ...
    try {
      packetPlayerChatConstructor = Class.forName("net.minecraft.server." + version + ".PacketPlayOutChat").getConstructor(Class
          .forName("net.minecraft.server." + version + ".IChatBaseComponent"), byte.class);
    } catch (final NoSuchMethodException e) {
      packetPlayerChatConstructor = Class.forName("net.minecraft.server." + version + ".PacketPlayOutChat").getConstructor(Class
          .forName("net.minecraft.server." + version + ".IChatBaseComponent"), Class.forName("net.minecraft.server." + version + ".ChatMessageType"));
    ...
    try {
      ...
      Class.forName("org.bukkit.craftbukkit." + version + ".entity.CraftPlayer").getMethod("getHandle").getReturnType()
          .getField("playerConnection").getType().getMethod("sendPacket", Class.forName("net.minecraft.server." + version +
          ".Packet")).invoke(connection, packetPlayerChatConstructor.newInstance(icb, (byte) 2));
    } catch (final IllegalAccessException | InstantiationException | InvocationTargetException | NoSuchMethodException |
